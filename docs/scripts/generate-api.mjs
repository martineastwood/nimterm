...
const sourceFile = join(packageDir, 'src', 'nimterm.nim');
...
function sourceLink(module, line) {
	...
	return `\n[View source](https://github.com/martineastwood/nimterm/blob/main/${sourcePath}#L${line})`;
}
...
function main() {
	const jsonDir = mkdtempSync(join(tmpdir(), 'nimterm-jsondoc-'));
	try {
		execFileSync('nim', [
			'jsondoc',
			'--raw',
