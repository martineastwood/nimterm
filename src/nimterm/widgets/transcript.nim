...
proc itemLines(item: transcript_model.TranscriptItem): seq[string] =
  ...
  of tikAssistant:
    result = markdown_renderer.renderMarkdown(item.text, false).splitLines
  of tikThinking:
    for line in item.text.splitLines: result.add "│ " & line
  of tikTool:
...
proc allLines(widget: TranscriptWidget): seq[tuple[text: string, style: Style]] =
  for item in widget.transcript.items:
    for line in item.itemLines:
      result.add (line, widget.itemStyle(item.kind))
...
method paint*(widget: TranscriptWidget, canvas: var Canvas) =
  ...
  for i in start ..< lines.len:
    ...
    canvas.writeText(widget.area.x, y, lines[i].text, lines[i].style,
      widget.area.w)
    inc y
